...
declare class keyEventTypes extends Event {
  on(event: string, listener: (event: S.KeyDownEevent) => any): void
  off(event: string, listener: (event: S.KeyDownEevent) => any): void
}
...
// let appHotKeyConfig: S.HotKeyConfigAll = window.s.appHotKeyConfig
...
export const registerKeyEvent = () => {
  keyBind.bindKey((key, eventKey, type, event, keys, isEditing) => {
    ...
    if (window.s.isEditingHotKey || (isEditing && type == 'down') || event?.s_handled) return
    if (event && window.s.appHotKeyConfig.local.enable && window.s.appHotKeyConfig.local.keys[key] && (key != 'escape' || !((event.target as HTMLElement).classList.contains('ignore-esc')))) {
      // console.log(key, eventKey, type, keys, isEditing)
      event.preventDefault()
      if (type == 'up') return

      // 软件内快捷键的最小化触发时
      // 如果已启用托盘，则隐藏程序，否则最小化程序 https://github.com/lyswhut/s-music-desktop/issues/603
      if (window.s.appHotKeyConfig.local.keys[key].action == HOTKEY_COMMON.min.action && appSetting['tray.enable']) {
        window.key_event.emit(HOTKEY_COMMON.hide_toggle.action)
        return
      }

      window.key_event.emit(window.s.appHotKeyConfig.local.keys[key].action)
      return
    }
    // console.log(`key_${key}_${type}`)
