...
export const deduplicationList = <T extends S.Music.MusicInfo>(list: T[]): T[] => {
...
export const langS2T = async(str: string) => {
  return window.s.worker.main.langS2t(Buffer.from(str).toString('base64')).then(b64 => Buffer.from(b64, 'base64').toString())
}
