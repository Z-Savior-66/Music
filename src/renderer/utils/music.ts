...
export const checkDownloadFileAvailable = async(musicInfo: S.Download.ListItem, savePath: string): Promise<boolean> => {
...
export const checkLocalFileAvailable = async(musicInfo: S.Music.MusicInfoLocal): Promise<boolean> => {
 ...
 * @param savePath
 */
export const checkMusicFileAvailable = async(musicInfo: S.Music.MusicInfo | S.Download.ListItem, savePath: string): Promise<boolean> => {
...
export const getDownloadFilePath = async(musicInfo: S.Download.ListItem, savePath: string): Promise<string> => {
...
export const getLocalFilePath = async(musicInfo: S.Music.MusicInfoLocal): Promise<string> => {
 ...
 * @returns
 */
export const getMusicFilePath = async(musicInfo: S.Music.MusicInfo | S.Download.ListItem, savePath: string): Promise<string> => {
  if ('progress' in musicInfo) {
 ...
 * @returns
 */
export const createLocalMusicInfo = async(path: string): Promise<S.Music.MusicInfoLocal | null> => {
...
let prevFileInfo: {
  path: string
  promise: Promise<S.MusicMetadataModule.IAudioMetadata | null>
} = {
  path: '',
...
//     }
//   }
//   const lyricInfo: S.Music.LyricInfo = {
//     lyric: lyrics[0].join('\n'),
//     tlyric: '',
//   }
 ...
 * @param path 路径
 */
export const getLocalMusicFileLyric = async(path: string): Promise<S.Music.LyricInfo | null> => {
  // 尝试读取同目录下的同名lrc文件
  const filePath = new RegExp('\\' + extname(path) + '$')
