...
const getListPlayIndex = (list: S.Music.MusicInfoOnline[], index?: number) => {
  if (index == null) {
    index = 1
  } else {
    if (index < 1) index = 1
...
export default () => {
  const playSongListDetail = async(source: S.OnlineSource, link: string, playIndex?: number) => {
  ...
  return async(source: S.OnlineSource, link: string, playIndex?: number) => {
    try {
