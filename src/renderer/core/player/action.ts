...
let gettingUrlId = ''
const createGettingUrlId = (musicInfo: S.Music.MusicInfo | S.Download.ListItem) => {
...
const createDelayNextTimeout = (delay: number) => {
  ...
  const addDelayNextTimeout = () => {
    ...
    timeout = setTimeout(() => {
      timeout = null
      if (window.s.isPlayedStop) return
 ...
 * 检查音乐信息是否已更改
 */
const diffCurrentMusicInfo = (curMusicInfo: S.Music.MusicInfo | S.Download.ListItem): boolean => {
...
const delayRetry = async(musicInfo: S.Music.MusicInfo | S.Download.ListItem, isRefresh = false): Promise<string | null> => {
...
const getMusicPlayUrl = async(musicInfo: S.Music.MusicInfo | S.Download.ListItem, isRefresh = false, isRetryed = false): Promise<string | null> => {
  ...
  return (toggleMusicInfo ? getMusicUrl({
    ...
    if (window.s.isPlayedStop || diffCurrentMusicInfo(musicInfo)) return null
...
export const setMusicUrl = (musicInfo: S.Music.MusicInfo | S.Download.ListItem, isRefresh?: boolean) => {
  // if (appSetting['player.autoSkipOnError']) addLoadTimeout()
  if (!diffCurrentMusicInfo(musicInfo)) return
  if (cancelDelayRetry) cancelDelayRetry()
...
// 恢复上次播放的状态
const handleRestorePlay = async(restorePlayInfo: S.Player.SavedPlayInfo) => {
  ...
  if (appSetting['player.togglePlayMethod'] == 'random' && !playMusicInfo.isTempPlay) addPlayedList({ ...playMusicInfo as S.Player.PlayMusicInfo })
}
...
const handlePlay = () => {
  ...
  if (window.s.restorePlayInfo) {
    void handleRestorePlay(window.s.restorePlayInfo)
...
const randomNextMusicInfo = {
  info: null as S.Player.PlayMusicInfo | null,
  // index: -1,
}
...
export const getNextPlayMusicInfo = async(): Promise<S.Player.PlayMusicInfo | null> => {
...
const handlePlayNext = (playMusicInfo: S.Player.PlayMusicInfo) => {
  // pause()
