...
const verifyQueryParams = async function(this: any, to: { query: Query, path: string }, from: any, next: (route?: { path: string, query: Query }) => void) {
  ...
  if (to.query.fromName) window.s.songListInfo.fromName = to.query.fromName
}
...
export default {
  ...
  setup() {
    ...
    const handleBack = () => {
      setVisibleListDetail(false)
      if (window.s.songListInfo.fromName) void router.replace({ name: window.s.songListInfo.fromName })
      else router.back()
    }
