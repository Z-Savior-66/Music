 ...
 * @returns 排序后的列表
 */
const handleSortList = (list: S.Music.MusicInfo[], keyword: string) => {
  let arr: any[] = []
...
const setLists = (results: SearchResult[], page: number, text: string): S.Music.MusicInfo[] => {
  let pages = []
  let totals = []
  let limit = 0
  let list = []
...
const setList = (datas: SearchResult, page: number, text: string): S.Music.MusicInfo[] => {
...
export const resetListInfo = (sourceId: S.OnlineSource | 'all'): [] => {
...
export const search = async(text: string, page: number, sourceId: S.OnlineSource | 'all'): Promise<S.Music.MusicInfo[]> => {
