...
export const setBoard = (board: Board, source: S.OnlineSource) => {
...
export const getBoardsList = async(source: S.OnlineSource) => {
...
export const getListDetail = async(id: string, page: number, isRefresh = false): Promise<ListDetailInfo> => {
  ...
  const [source, bangId] = id.split('__') as [S.OnlineSource, string]
 ...
 * @returns
 */
export const getListDetailAll = async(id: string, isRefresh = false): Promise<S.Music.MusicInfoOnline[]> => {
  const [source, bangId] = id.split('__') as [S.OnlineSource, string]
