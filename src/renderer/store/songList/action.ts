...
export const setTags = (tagInfo: TagInfo, source: S.OnlineSource) => {
...
export const getTags = async<T extends S.OnlineSource>(source: T) => {
 ...
 * @returns
 */
export const getAndSetList = async(source: S.OnlineSource, tabId: string, sortId: string, page: number, isRefresh = false) => {
 ...
 * @returns
 */
export const getListDetail = async(id: string, source: S.OnlineSource, page: number, isRefresh = false): Promise<ListDetailInfo> => {
  ...
  return musicSdk[source]?.songList.getListDetail(id, page).then((result: ListDetailInfo) => {
    result.list = markRawList(deduplicationList(result.list.map(m => toNewMusicInfo(m)) as S.Music.MusicInfoOnline[]))
    cache.set(key, result)
    return result
  })
}
 ...
 * @returns
 */
export const getListDetailAll = async(id: string, source: S.OnlineSource, isRefresh = false): Promise<S.Music.MusicInfoOnline[]> => {
 ...
 * @returns
 */
export const getAndSetListDetail = async(id: string, source: S.OnlineSource, page: number, isRefresh = false) => {
  let key = `sdetail__${source}__${id}__${page}`
