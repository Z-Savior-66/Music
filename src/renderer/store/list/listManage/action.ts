...
export const setUserLists = (lists: S.List.UserListInfo[]) => {
  userLists.splice(0, userLists.length, ...lists)
  return userLists
}

export const setMusicList = (listId: string, musicList: S.Music.MusicInfo[]) => {
  const list = markRawList(musicList)
  allMusicList.set(listId, list)
  return list
}

const overwriteMusicList = (id: string, list: S.Music.MusicInfo[]) => {
  // console.log(id, list)
  markRawList(list)
...
const createUserList = ({
  ...
  sourceListId,
  locationUpdateTime,
}: S.List.UserListInfo, position: number) => {
...
const updateList = ({
  ...
  meta,
  locationUpdateTime,
}: S.List.UserListInfo & { meta?: { id?: string } }) => {
  let targetList
  switch (id) {
    case defaultList.id:
    case loveList.id:
...
const overwriteUserList = (lists: S.List.UserListInfo[]) => {
  userLists.splice(0, userLists.length, ...lists)
}
...
export const listDataOverwrite = ({ defaultList, loveList, userList, tempList }: MakeOptional<S.List.ListDataFull, 'tempList'>): string[] => {
...
export const userListsUpdate = (listInfos: S.List.UserListInfo[]) => {
...
export const userListsUpdatePosition = (position: number, ids: string[]) => {
  ...
  const updateLists: S.List.UserListInfo[] = []

  // const targetItem = list[position]
  const map = new Map<string, S.List.UserListInfo>()
...
export const listMusicOverwrite = (listId: string, musicInfos: S.Music.MusicInfo[]): string[] => {
...
export const listMusicAdd = (id: string, musicInfos: S.Music.MusicInfo[], addMusicLocationType: S.AddMusicLocationType): string[] => {
...
export const listMusicMove = (fromId: string, toId: string, musicInfos: S.Music.MusicInfo[], addMusicLocationType: S.AddMusicLocationType): string[] => {
  return [
...
export const listMusicUpdateInfo = (musicInfos: S.List.ListActionMusicUpdate): string[] => {
  ...
  for (const { id, musicInfo } of musicInfos) {
    ...
    if (index < 0) continue
    const info: S.Music.MusicInfo = { ...targetList[index] }
    Object.assign(info, {
      name: musicInfo.name,
...
export const listMusicUpdatePosition = async(listId: string, position: number, ids: string[]): Promise<string[]> => {
  ...
  const list = await window.s.worker.main.createSortedList(toRaw(targetList), position, ids)
  markRawList(list)
  targetList.splice(0, targetList.length)
  arrPush(targetList, list)
