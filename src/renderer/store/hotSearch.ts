...
export type Source = S.OnlineSource | 'all'

interface SourceLists extends Partial<Record<S.OnlineSource, string[]>> {
  'all': string[]
}
...
for (const source of music.sources) {
  if (!music[source.id as S.OnlineSource]?.hotSearch) continue
  sources.push(source.id as S.OnlineSource)
  sourceList[source.id as S.OnlineSource] = reactive<string[]>([])
}
sources.push('all')


const setList = (source: S.OnlineSource, list: string[]): string[] => {
  return sourceList[source] = list.slice(0, 20)
}

const setLists = (lists: Array<{ source: S.OnlineSource, list: string[] }>): string[] => {
