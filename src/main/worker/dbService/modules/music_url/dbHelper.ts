 ...
 * @param urlInfo 列表
 */
export const insertMusicUrl = (urlInfo: S.DBService.MusicUrlInfo[]) => {
  const db = getDB()
  const insertStatement = createInsertStatement()
  const deleteStatement = createDeleteStatement()
  db.transaction((urlInfo: S.DBService.MusicUrlInfo[]) => {
 ...
 * @param urlInfo 列表
 */
// export const updateMusicUrl = (urlInfo: S.DBService.MusicUrlInfo[]) => {
//   const db = getDB()
