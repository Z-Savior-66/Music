...
export const createListInsertStatement = () => {
  const db = getDB()
  return db.prepare<[S.DBService.UserListInfo]>(`
...
export const createMusicInfoQueryStatement = () => {
  const db = getDB()
  return db.prepare<[S.DBService.MusicInfoQuery]>(`
...
export const createMusicInfoInsertStatement = () => {
  const db = getDB()
  return db.prepare<[S.DBService.MusicInfo]>(`
...
export const createMusicInfoDeleteStatement = () => {
  const db = getDB()
  return db.prepare<[S.DBService.MusicInfoRemove]>('DELETE FROM "main"."my_list_music_info" WHERE "id"=@id AND "listId"=@listId')
}
...
export const createMusicInfoByListAndMusicInfoIdQueryStatement = () => {
  const db = getDB()
  return db.prepare<[S.DBService.ListMusicInfoQuery]>(`SELECT "id", "name", "singer", "source", "interval", "meta"
...
export const createMusicInfoOrderInsertStatement = () => {
  const db = getDB()
  return db.prepare<[S.DBService.MusicInfoOrder]>(`
...
export const createMusicInfoOrderDeleteStatement = () => {
  const db = getDB()
  return db.prepare<[S.DBService.MusicInfoRemove]>('DELETE FROM "main"."my_list_music_info_order" WHERE "musicInfoId"=@id AND "listId"=@listId')
}
