...
const verifyLyricInfo = (info) => {
  if (typeof info != 'object' || typeof info.lyric != 'string') throw new Error('返回歌词格式无效')
  if (info.lyric.length > 51200) throw new Error('返回歌词内容过长')
  return {
    lyric: info.lyric,
...
const handleRequest = (context, { requestKey, data }) => {
  if (!events.request) return sendMessage(USER_API_RENDERER_EVENT_NAME.response, { requestKey }, false, '音源未注册请求事件')
  try {
    events.request.call(context, { source: data.source, action: data.action, info: data.info }).then(response => {
      ...
      switch (data.action) {
        case 'musicUrl':
          if (typeof response != 'string' || response.length > 2048 || !/^https?:/.test(response)) throw new Error('返回播放链接无效')
          sendData.result = {
            source: data.source,
            action: data.action,
            data: {
        ...
        case 'lyric':
          ...
          break
        case 'pic':
          if (typeof response != 'string' || response.length > 2048 || !/^https?:/.test(response)) throw new Error('返回图片链接无效')
          sendData.result = {
            source: data.source,
            action: data.action,
            data: response,
...
const handleInit = (context, info) => {
  if (!info) {
    sendMessage(USER_API_RENDERER_EVENT_NAME.init, null, false, '缺少音源初始化信息')
    return
  }
  ...
  try {
    for (const source of allSources) {
      ...
      sourceInfo.sources[source] = {
        type: 'music',
        actions: actions.filter(action => userSource.actions.includes(action)),
  ...
  contextBridge.exposeInMainWorld('lx', {
    ...
    send(eventName, data) {
      return new Promise((resolve, reject) => {
        if (!eventNames.includes(eventName)) return reject(new Error('不支持此事件：' + eventName))
        switch (eventName) {
          case EVENT_NAMES.inited:
            if (isInitedApi) return reject(new Error('音源脚本已初始化'))
            isInitedApi = true
            handleInit(this, data)
            resolve()
            break
          case EVENT_NAMES.updateAlert:
            if (isShowedUpdateAlert) return reject(new Error('更新提醒只能调用一次'))
    ...
    on(eventName, handler) {
      if (!eventNames.includes(eventName)) return Promise.reject(new Error('不支持此事件：' + eventName))
      switch (eventName) {
        case EVENT_NAMES.request:
          events.request = handler
          break
        default:
          return Promise.reject(new Error('不支持此事件：' + eventName))
      }
