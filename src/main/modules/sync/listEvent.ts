...
// 构建列表信息对象，用于统一字段位置顺序
export const buildUserListInfoFull = ({ id, name, source, sourceListId, list, locationUpdateTime }: S.List.UserListInfoFull) => {
  return {
    id,
    name,
    source,
...
export const getLocalListData = async(): Promise<S.Sync.List.ListData> => {
  const lists: S.Sync.List.ListData = {
    defaultList: await global.s.worker.dbService.getListMusics(LIST_IDS.DEFAULT),
    loveList: await global.s.worker.dbService.getListMusics(LIST_IDS.LOVE),
    userList: [],
  }

  const userListInfos = await global.s.worker.dbService.getAllUserList()
  for await (const list of userListInfos) {
    lists.userList.push(await global.s.worker.dbService.getListMusics(list.id)
...
export const setLocalListData = async(listData: S.Sync.List.ListData) => {
  await global.s.event_list.list_data_overwrite(listData, true)
}


export const registerListActionEvent = (sendListAction: (action: S.Sync.List.ActionList) => (void | Promise<void>)) => {
  const list_data_overwrite = async(listData: MakeOptional<S.List.ListDataFull, 'tempList'>, isRemote: boolean = false) => {
    if (isRemote) return
    await sendListAction({ action: 'list_data_overwrite', data: listData })
  }
  const list_create = async(position: number, listInfos: S.List.UserListInfo[], isRemote: boolean = false) => {
    if (isRemote) return
  ...
  const list_update = async(lists: S.List.UserListInfo[], isRemote: boolean = false) => {
    if (isRemote) return
  ...
  const list_music_overwrite = async(listId: string, musicInfos: S.Music.MusicInfo[], isRemote: boolean = false) => {
    if (isRemote || listId == LIST_IDS.TEMP) return
    await sendListAction({ action: 'list_music_overwrite', data: { listId, musicInfos } })
  }
  const list_music_add = async(id: string, musicInfos: S.Music.MusicInfo[], addMusicLocationType: S.AddMusicLocationType, isRemote: boolean = false) => {
    if (isRemote) return
    await sendListAction({ action: 'list_music_add', data: { id, musicInfos, addMusicLocationType } })
  }
  const list_music_move = async(fromId: string, toId: string, musicInfos: S.Music.MusicInfo[], addMusicLocationType: S.AddMusicLocationType, isRemote: boolean = false) => {
    if (isRemote) return
  ...
  const list_music_update = async(musicInfos: S.List.ListActionMusicUpdate, isRemote: boolean = false) => {
...
export const handleRemoteListAction = async({ action, data }: S.Sync.List.ActionList) => {
  // console.log('handleRemoteListAction', action)

  switch (action) {
    case 'list_data_overwrite':
      await global.s.event_list.list_data_overwrite(data, true)
      break
    case 'list_create':
      await global.s.event_list.list_create(data.position, data.listInfos, true)
      break
    case 'list_remove':
      await global.s.event_list.list_remove(data, true)
      break
    case 'list_update':
      await global.s.event_list.list_update(data, true)
      break
    case 'list_update_position':
      await global.s.event_list.list_update_position(data.position, data.ids, true)
      break
    case 'list_music_add':
      await global.s.event_list.list_music_add(data.id, data.musicInfos, data.addMusicLocationType, true)
      break
    case 'list_music_move':
      await global.s.event_list.list_music_move(data.fromId, data.toId, data.musicInfos, data.addMusicLocationType, true)
      break
    case 'list_music_remove':
      await global.s.event_list.list_music_remove(data.listId, data.ids, true)
      break
    case 'list_music_update':
      await global.s.event_list.list_music_update(data, true)
      break
    case 'list_music_update_position':
      await global.s.event_list.list_music_update_position(data.listId, data.position, data.ids, true)
      break
    case 'list_music_overwrite':
      await global.s.event_list.list_music_overwrite(data.listId, data.musicInfos, true)
      break
    case 'list_music_clear':
      await global.s.event_list.list_music_clear(data, true)
      break
    default:
