...
export const registerRendererEvents = (sendEvent: <T = any>(name: string, params?: T | undefined) => void) => {
  const dislike_music_add = async(listData: S.Dislike.DislikeMusicInfo[]) => {
    sendEvent<S.Dislike.DislikeMusicInfo[]>(DISLIKE_EVENT_NAME.add_dislike_music_infos, listData)
  }
  const dislike_data_overwrite = async(rules: S.Dislike.DislikeRules) => {
    sendEvent<S.Dislike.DislikeRules>(DISLIKE_EVENT_NAME.overwrite_dislike_music_infos, rules)
  }
